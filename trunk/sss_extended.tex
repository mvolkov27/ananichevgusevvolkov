...
\end{picture}
\end{center}
\caption{ Автомат $\mathrsfs{F}$ $n^2 - 3n + 2$}
...
\begin{theorem}
Автомат $\mathrsfs{F}$ с картинки \ref{fig:aut:shortcycle} не удастся синхронизировать быстрее, чем за $n^2 - 3n + 2$.
\end{theorem}
\begin{proof}
...
Пусть $w$ -- некоторое синхронизирующее слово автомата $\mathrsfs{F}$ минимальной длины. Нетрудно проверить, 
...
При этом, полученная оценка точна, поскольку слово $(aab^{n - 2})^{n - 3}a^2$ синхронизирует автомат $\mathrsfs{F}$.~$\square$
...
\end{picture}
\end{center}
\caption{ Автомат $\mathrsfs{A}$ $n^2 - 3n + 3$, $n$ нечетно}
...
\begin{theorem}
Граница синхронизации автомата, изображенного на рис.~\ref{fig:aut:middleisland} равна $n^2 - 3n + 3$.
\end{theorem}

\begin{proof}
Пусть $w$ кратчайшее синхронизирующее слово автомата $\mathrsfs{A}$, изображенного на рис.~\ref{fig:aut:middleisland}. 
...
вхождение $ab$ на $d$. Легко видеть, что $ab$ в автомате $\mathrsfs{A}$ действует также, как и $d$ в $\mathrsfs{D}$.
...
По рассуждениям для автоматов с длинным циклом [диплом] имеем, что длина слова $vd$ никак не меньше $(n - 2)n - (2n - 2) + 1 = n^2 - 4n + 3$,
...
\end{picture}
\end{center}
\caption{Автомат $\mathrsfs{A}$, $n^2 - 4n + 6$}
...
\begin{theorem}
Граница синхронизации автомата с рис. \ref{fig:aut:evenidem} равна $n^2 - 4n + 6$.
\end{theorem}
\begin{proof}
Пусть $w$ кратчайшее синхронизирующее слово автомата $\mathrsfs{A}$, изображенного на рис.~\ref{fig:aut:evenidem}.
Преобразование, соответствующее букве $a$, является идемпотентом,
...
%$w = aw'$ и в $w'$ всякое появление $a$ предварено $b$.
Введем новую букву $d = ab$, тогда $w$ можно однозначно преобразовать в слово $v$ над $\{b,d\}$, заменив всякое 
вхождение $ab$ на $d$. Легко видеть, что $ab$ в автомате $\mathrsfs{A}$ действует также, как и $d$ в $\mathrsfs{D}$, но тогда
$v$ синхронизирующее слово для автомата $\mathrsfs{D}$.
...
а все остальные заняты. Таким образом, нам необходимо синхронизировать автомат Виланда на $(n - 1)$ состояниях,
что может быть сделано за $(n - 1)^2 - 3(n - 1) + 3 = n^2 - 5n + 7$ шагов[диплом], и тогда длина $v$ не меньше $n^2 - 5n + 8$.
...
слово $b(ab^{n - 2})^{n - 3}ab$ синхронизирует автомат $\mathrsfs{A}$.~$\square$
\end{proof}
...
\end{picture}
\end{center}
\caption{ Автомат $n^2 - 4n + 7$, $n$ нечетно}
\label{fig:aut:fat}
\end{figure}

\begin{theorem}
Автомат на картинке \ref{fig:aut:fat} имеет границей синхронизации $n^2 - 4n + 7$.
\end{theorem}
