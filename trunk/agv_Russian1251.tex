...
алгоритм проверки синхронизируемости очень быстр, так как работает с графом пар
состояний, в то время как алгоритм вычисления порога синхронизации работает с
графом непустых подмножеств состояний и в наихудшем случае время его работы
...
сходно с поведением количества примитивных орграфов с фиксированным числом
вершин как функции от значения экспоненты. В следующем параграфе мы обсудим это
сходство подробнее, а в \S4 дадим его частичное объяснение.
...
предполагаем, что читатель знаком с основными понятиями теории графов, такими
как маршрут, путь, контур, изоморфизм, и т.\,д.
...
\label{schur} Пусть $k_1,\dots,k_m$ -- некоторый набор натуральных чисел, чей
наибольший общий делитель равен еденице. Тогда найдется натуральное число $N$
такое, что всякое число превосходящее $N$ представимо в виде целой
неотрицательной линейной комбинации $k_1,\dots,k_m$.
\end{lemma}
